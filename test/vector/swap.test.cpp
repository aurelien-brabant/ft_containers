...
TEST(test_vector_swap)
{
    ...
    vector<int>::size_type v1_old_cap = v1.capacity(), v1_old_len = v1.size(),
                           v2_old_cap = v2.capacity(), v2_old_len = v2.size();

    vector<int>::pointer v1_old_data = v1.data(), v2_old_data = v2.data();
    ...
    for (vector<int>::const_iterator cit = v1.begin(), old = first2;
         cit != v1.end();
         ++cit, ++old) {
        assert_expr(cit == old);
    }

    for (vector<int>::const_iterator cit = v2.begin(), old = first1;
         cit != v2.end();
         ++cit, ++old) {
        assert_expr(cit == old);
    }
